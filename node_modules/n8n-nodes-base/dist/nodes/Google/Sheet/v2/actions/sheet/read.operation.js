...
async function execute(sheet, sheetName) {
    ...
    let responseData = [];
    const lookupValues = this.getNodeParameter('filtersUI.values', 0, []);
    if (lookupValues.length) {
        ...
        responseData = await sheet.lookupValues(data, headerRow, firstDataRow, lookupValues, returnAllMatches);
    }
    else {
    ...
    const returnData = responseData.map((item, index) => {
        return {
            json: item,
            pairedItem,
        };
