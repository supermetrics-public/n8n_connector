...
class Ldap {
        ...
        this.methods = {
            credentialTest: {
                async ldapConnectionTest(credential) {
                    ...
                    const client = await (0, Helpers_1.createLdapClient)(credentials);
                    try {
    ...
    async execute() {
        ...
        const returnItems = [];
        if (nodeDebug) {
            n8n_workflow_1.LoggerProxy.info(`[${this.getNode().type} | ${this.getNode().name}] - Starting with ${items.length} input items`);
        }
        const credentials = await this.getCredentials('ldap');
        const client = await (0, Helpers_1.createLdapClient)(credentials, nodeDebug, this.getNode().type, this.getNode().name);
        try {
            await client.bind(credentials.bindDN, credentials.bindPassword);
        }
        catch (error) {
        ...
        for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
            try {
                if (operation === 'compare') {
                    ...
                    if (nodeDebug) {
                        n8n_workflow_1.LoggerProxy.info(`[${this.getNode().type} | ${this.getNode().name}] - Search Options ${JSON.stringify(options, null, 2)}`);
                    }
        ...
        if (nodeDebug) {
            n8n_workflow_1.LoggerProxy.info(`[${this.getNode().type} | ${this.getNode().name}] - Finished`);
        }
        await client.unbind();
        return [returnItems];
    }
